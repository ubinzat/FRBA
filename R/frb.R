...
frb <- function(lmrob.object, nboot=1000, return.coef = FALSE, return.indices = FALSE, centered=TRUE,varr=1) {
  varri=varr
  lmrob.Chi <- Mchi
  lmrob.Psi <- Mpsi
  ...
  a <- .C('R_frb', as.double(xx), as.double(yy), as.double(w), as.integer(n), 
          ...
          bind = as.integer(boot.indices), as.integer(varr), PACKAGE="FRB")
